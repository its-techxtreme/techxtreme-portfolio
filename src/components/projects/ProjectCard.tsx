      ...
      <div className="absolute inset-0 flex flex-col justify-end bg-gradient-to-t from-bg via-bg/80 to-transparent p-7 dark:from-bg dark:via-bg/80 light:from-[#0f2433] light:via-[#0f2433]/90 light:to-transparent light:text-white">
        <div className="mb-2 flex flex-wrap gap-2">
          {project.tags.slice(0, 3).map((t) => (
            <span key={t} className={clsx("rounded px-2 py-0.5 font-mono text-[0.7rem] uppercase tracking-wider sm:text-xs", tagClass(t))}>
              {t}
            </span>
          ))}
        </div>
  ...
  return (
    ...
    >
      <Link to={`/work/${project.slug}`}>
        <div className={clsx("relative overflow-hidden", project.featured ? "aspect-video md:aspect-[21/9]" : "aspect-video")}>
          <div className="absolute left-4 top-4 z-10 flex flex-wrap gap-2">
            {project.tags.slice(0, 2).map((t) => (
              <span
                key={t}
                className={clsx(
                  "rounded border border-line bg-bg/80 px-2 py-1 font-mono text-[0.7rem] uppercase backdrop-blur sm:text-xs",
                  tagClass(t)
                )}
              >
                {t}
        ...
        <div className="p-5 sm:p-7 light:bg-white">
